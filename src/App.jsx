...
const Services = ({ isDark }) => {
  const servicesData = [
    ...
    },
    {
      ...
      img: "/images/logistics.jpg" // Fixed typo here
    }
  ];
...
// 7.5 Blog Section (Restored & Updated)
const BlogSection = ({ isDark }) => {
  ...
  return (
    <section id="blogs" className={`py-20 md:py-32 relative z-10 ${isDark ? 'bg-zinc-950' : 'bg-zinc-50'}`}>
      <div className="max-w-7xl mx-auto px-6">
        <h2 className={`font-artistic text-3xl md:text-4xl font-bold mb-12 ${isDark ? 'text-white' : 'text-zinc-900'}`}>Insights & Resources</h2>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {topics.map((topic, i) => (
            <div key={i} className={`group cursor-pointer border p-6 hover:shadow-lg transition-all ${isDark ? 'border-zinc-800 bg-black hover:border-zinc-600' : 'border-zinc-200 bg-white hover:border-zinc-400'}`}>
              <div className="flex justify-between items-start mb-4">
                <FileText className={`w-6 h-6 ${isDark ? 'text-gray-500' : 'text-zinc-400'}`} />
                <span className="text-[10px] font-mono uppercase text-gray-500">Vol. {i + 1}</span>
              </div>
              <h3 className={`text-lg font-bold group-hover:underline decoration-1 underline-offset-4 ${isDark ? 'text-white' : 'text-zinc-900'}`}>{topic}</h3>
            </div>
          ))}
        </div>
      </div>
